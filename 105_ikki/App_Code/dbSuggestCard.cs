...
public class dbSuggestCard:DBBaseClass
{
    ...
    public DataSet Sel_SC_ParameterItems(string strStoreNo, out string sReturnMsg)
    {
        ...
        try
        {
            ...
            sb.Append(@" WHERE PTypeID='" + strStoreNo + "Times'");
    ...
    /// <param name="sRecMainGUID">RecMainGUID</param>0dfd3bff-afe9-4c22-bdd7-138df3223314
    ...
    public bool InsertSSF_Daily_OtherSuggest(string sMainDataGUID, string sRecMainGUID, string sVersionID, string sVersionName, string sStoreNo, string sExpendDate, string sCName, string sDOGUID, string sSugType, string sSuggest, string sAnalyMGUID, string sAnalyDGUID, string sSugDetail)
    {
        ...
        WHERE DOGUID= @DOGUID
            ...
            SQC.Parameters.AddWithValue("VersionID", sVersionID);
            SQC.Parameters.AddWithValue("VersionName", sVersionName);
            SQC.Parameters.AddWithValue("AnalyMGUID", sAnalyMGUID.Trim());//意見分類 唯一值!!
            SQC.Parameters.AddWithValue("AnalyDGUID", sAnalyDGUID.Trim());//意見菜色 唯一值!!
            SQC.Parameters.AddWithValue("SugMGUID", sSugType);            //建議方向
            SQC.Parameters.AddWithValue("SugDGUID", sSugDetail);          //意見細節 唯一值!!
            SQC.Parameters.AddWithValue("SugTypeMNo", sSugType);          //建議方向 
            SQC.Parameters.AddWithValue("SugTypeDNo", sSugDetail);        //意見細節 唯一值!!
            SQC.Parameters.AddWithValue("SugCount", 1);
            SQC.Parameters.AddWithValue("SugNote", sSuggest);
            SQC.Parameters.AddWithValue("InsertMan", sCName);
    ...
    /*  [ProdSugGUID]--key值
		...
		[CareerNo]   --事業處
		[iType]      --用途 */
    public bool InsertSC_ProductSugguest(string sProdSugGUID, string sRecMainGUID, string sAnsGUID, string sAnsName, string sSugNote, string sCareerNo, string siType)
    {
        bool blReturn;
        try
        {
            ...
            IF NOT EXISTS (SELECT *  FROM [dbo].[SC_ProductSugguest] Where ProdSugGUID= @ProdSugGUID)
									BEGIN
										INSERT INTO [SC_ProductSugguest] (ProdSugGUID,RecMainGUID,AnsGUID,AnsName,SugNote,CareerNo,iType,InsertTime)
									                              VALUES(@ProdSugGUID,@RecMainGUID,@AnsGUID,@AnsName,@SugNote,@CareerNo,@iType,GETDATE())
									END 
                                    ELSE
										BEGIN
										UPDATE [SC_ProductSugguest] 
										SET RecMainGUID=@RecMainGUID,AnsGUID=@AnsGUID,AnsName=@AnsName,SugNote=@SugNote,CareerNo=@CareerNo,iType=@iType,InsertTime=GETDATE()
